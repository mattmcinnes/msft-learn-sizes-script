...
$defaultArticlesGitDir = "C:\Users\$env:USERNAME\GitHub\azure-compute-docs-pr"

## Function to delay and show dots
function ShortDelayDots {
...
$userResponse = Read-Host "Do you want to check for updates to the sizes script?`n(y/n)"
while ($true) {
    if ($userResponse -eq "y") {
        $runScriptUpdate = $true
        break
    } elseif ($userResponse -eq "n") {
        Write-Host "`nSkipping update check" -ForegroundColor Yellow -NoNewline
        ShortDelayDots
        Exit
    } else {
